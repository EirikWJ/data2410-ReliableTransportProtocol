import argparse #imports the arg parse 
from server import Server #imports the server code to the application file
from client import Client #imports the client code to the application file 
from utilityFunctions import * #imports all the util functions
...
def windowValid(args):
    if args.window < 0:#exp can't be using a window with negative size now can we?
...
def fileValid(args):
    if args.server and args.file:
        print('Invalid direction of filetransfer.')#exp may only specify sent file when starting in client mode
        return False
    if not args.file and args.client:#exp   is there is no file specified. return
...
def discardValid(args):
    if args.discard < 0:#exp    can't be discarding a packet with negative seq number now can we?
        print('Invalid seqno for packet to be discarded. Must be greater than 0')
        return False
    if args.client and args.discard:#exp    the packet gets dropped on the server side. not on the client
...
def processInputs(args):#takes in the argument list
    ...
    if not windowValid(args): return#false window size
    if not discardValid(args): return #false seq-num of intentional drop
    if not fileValid(args): return#false filename

    print(colours.GREEN+f'\n[{mode.upper()}] Running with IP = {args.ip} and port = {args.port}\n\n'+colours.WHITE)
    if args.server: Server(args)#starts the server
    if args.client: Client(args)#starts the client
...
#adds argument of window size, defualt size is 3
parser.add_argument("-w", "--window", type=int, default=3)
#adds argument of file to be transferred. required, but only on the client side
parser.add_argument("-f", "--file", type=str)
#adds argument of being able to specify what packet to discard. By default false (no intentional packet drop)
